    ...
    private inner class InlinerTypeRemapper(
        ...
        fun remapTypeAndOptionallyErase(type: IrType, erase: Boolean): IrType {
            ...
            if (erase && substitutedType != null && (classifier as? IrTypeParameterSymbol)?.owner?.isReified == false) {
                // Erase non-reified type parameter.
                val upperBound = remapTypeAndOptionallyErase(
                    classifier.owner.superTypes.firstOrNull() ?: TODO(classifier.owner.render()),
                    erase = true
                )
                return if (type.hasQuestionMark) upperBound.makeNullable() else upperBound
            }
